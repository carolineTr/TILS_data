...
#        pvaltxt <- ifelse(pvalue < 0.0001,"p < 0.0001",paste("p =0.000131"))
    # MOVE P-VALUE LEGEND HERE BELOW [set x and y]
#    p <- p + annotate("text",x = 150, y = 0.1,label = pvaltxt)
    p <- p + annotate("text",x = 100, y = 0.8,label = pvaltxt,size=5)
  }#if
  ...
  if(table) {
    ...
    data.table <- ggplot(risk.data,aes(x = time, y = strata, label = format(n.risk, nsmall = 0))) +
      ...
      theme(axis.title.x = element_text(size = 15, vjust = 1),
            ...
            axis.ticks = element_blank(),axis.text.y = element_text(face = "bold",hjust = 1,size=12))
